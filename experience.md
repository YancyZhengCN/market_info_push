...
- **✅ 2 日线相位已按各标的上市首日锚定、逐日对齐东财**（原「已知局限」已修复，见 §9.5）。
...
- **注意反直觉点（09-07 当时的结论，已被 §9.5 细化）**：那天沪深300 相位恰为「今日单独成一根」，
  故「今天+昨天配对」算错（-12.25）；但这**并非通则**——是否配对取决于各标的相位（见 §9.5）。
  09-08 沪深300 相位翻转为「今日与昨日配对」，此时反而必须配对才对齐东财。

### 9.5 2 日线相位对齐东财（各标的按上市首日锚定）

- **现象（09-08 复现）**：修好收盘口径后，2 日 BAR 仍**每隔一天就和东财差一根**。沪深300 09-08
  算出 -6.63 / 上 -9.67，东财 -8.00 / -9.96；而 09-07 是吻合的。
- **根因**：东财 2 日 K 线从**各标的上市首日**起固定两两分组，相位由「上市首日→今日」交易日数奇偶决定，
  **每个标的不同**（实测沪深300 相位为奇、创业板50/北证50 为偶）。§9.4 的「固定末根单独」只在半数交易日
  与东财同相位，另一半差一根。用户敏锐地猜到「东财以某种固定日起算」，方向正确——只是不是「奇数日」而是「上市首日」。
- **验证**：用全历史「从首日 //2 分组、取组内较新」复刻，逐值命中东财（09-08 沪深300 DIF/DEA/BAR
  = -51.40/-47.40/-8.00 vs 东财 -51.41/-47.42/-8.00，上根 -9.97 vs -9.96；创业板50/北证50/科创50 同样命中）。
- **修复（不拉全历史）**：本项目为性能只拉 3 年、拿不到首日。改为**离线标定相位锚点**：
  `macd._2D_PHASE_ANCHOR[ts_code] = (锚点交易日, 该日相位)`，运行时 `_last_is_paired` 用「锚点→末行」的
  交易日步数推算末行相位（每过 1 个交易日翻转），决定末根「配对/单独」。有界窗口 tail(730) 只要含锚点日即可，
  经跨日回测与「全历史真值」逐日一致。锚点不在窗口/标的无锚点时回退「末根单独」（相位可能差一根，安全降级）。
- **锚点标定方法（tools 离线一次性）**：对每个标的拉全历史，末行 0-based index 的奇偶即该末日相位；
  把 `(末日, 末idx%2)` 写入 `_2D_PHASE_ANCHOR`。新增 `basis=2d` 或需展示 2 日列的标的时补标一次。
  锚点日期本身不必是“今天”，任一窗口内交易日均可（推算只看相对步数）。
- **踩坑教训**：「从最新一根锚定」看似省事，但相位会随序列奇偶漂移，只对一半日子——**相位必须绑定
  一个与数据窗口无关的绝对基准（上市首日/固定锚点），不能绑定「最新一根」或「序列长度」**。
