...
- **⚠️ 2 日线相位与东财并非天天一致（已知局限）**。东财的 2 日 K 线是从**指数上市首日**固定两两分组
  （每 2 个交易日一根、收盘取组内较新一天）；本实现是**从最新一根锚定**向前两两配对。当「上市首日→今日」的
  交易日总数为奇数时两者分组一致（如 09-07 含实时价、09-03），为偶数时会错位一根（如 09-04 无实时价：
  本实现 -13.11 vs 东财起点锚定 -9.97）。**盘中早报场景**（拼入今日实时价、末日=今日）在多数交易日恰为奇数、
  与东财一致；若要严格逐日对齐东财，需按各标的真实上市首日固定分组相位（当前 akshare 有界窗口拿不到首日，
  且部分标的如北证50 历史本就不足）。
...
- **注意反直觉点**：「今天的 2 日 K 线 = 今天+昨天配对」是错的——实测这么配算出 -12.25，接近旧错误值；
  东财是把**今天单独**当作正在形成的一根 2 日 K 线，再往前两两合成。
