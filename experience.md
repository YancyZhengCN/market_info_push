...
- 增强版的周BAR放行 与 牛市的 EMA50/斜率**必须复用同一"已完成周线"边界**，否则两者对"本周算不算数"不一致。
...
- 牛市：周收盘须 `> EMA50×(1+buffer)`、EMA50 须 `> N周前`（等于都不算牛市）；增强版 BUY 须 `ΔBAR₂D > 0.4σ20`（等于算 HOLD）。
- 数据不足算不出 EMA50/N周斜率时牛市返回 `UNKNOWN`，**不得默认 NON_BULL**（否则会误把新标的判成非牛市而错误卖出）。
...
## 14. 牛市条件参数调整（2026-09-19）

> 需求见根目录 `market_info_push_牛市条件调整说明.md`；只调牛市判定参数，不动增强版/2日相位/状态机/卡片。

- 参数变化：价格缓冲 `2% → 0%`、EMA 斜率回看 `8周 → 12周`（EMA 周期仍 50、`adjust=False` 不变）。
  即新规则 = `已完成周收盘 > EMA50 且 EMA50 > 12周前EMA50`（均严格大于）。仅改 `config.py` 三个常量即可切换/回滚。
- **字段去参数绑定**：`BullMarketDecision.ema50_8w_ago` 改为 `ema50_slope_reference`，并加 `slope_lookback_weeks`/`price_buffer`。
  这样参数变化时字段名不再说谎；`main.py` 日志的"N周前EMA50"用 `slope_lookback_weeks` 动态渲染，回滚也不用改日志。
  **教训**：dataclass 字段名里写死"8周"这种参数值，一改参数就得全局改字段名、日志、测试；命名要对参数中立。
...
### 15.1 微信窄屏单元格换行：用 WORD JOINER(U+2060) 禁止断行（重要通用手段）

- **现象**：信号列 `-/-`、`🐮/⭕️` 在手机微信被折成两行（`-` 换行到 `/-`）。同屏价格 `4507.39` 不被拆断、只在全角括号 `（` 处换行。
- **根因**：微信富文本按 **Unicode 标准断行算法**换行，不是"逐字强断"。半角 `/`（SY）、`-`（HY）、全角括号（CJK）都是**合法断行点**，窄列宽下就在这些点折行。
- **解法（首选）**：在不想被拆断的字符之间插入 **WORD JOINER `\u2060`**（零宽、`GL` 类、明确"此处禁止换行"）。
  本项目 `_fmt_strategy_signals` 输出 `f"{bull}\u2060/\u2060{enh}"`，视觉仍是 3 个可见字符，但斜杠不再是断行点，整列不折行。
- **为什么不用 `&nbsp;`/`<nobr>`/`<br>`**：Server酱 markdown 不解析 HTML 标签（见 §11），只能靠 Unicode 字符本身控制断行。
- **通用规则**：以后凡"短 token 在微信/Server酱表格里被从中间拆行"，一律用 `\u2060` 夹在断点两侧消除断行点；
  测试里比对可见内容时先 `.replace("\u2060","")` 剥离，不要把零宽字符写进期望值。
  备选（WJ 万一不生效）：①把半角 `/`/`-` 换成不易断行的字符；②去掉分隔符只留双图标。
